...
class ms_report_piutang_xlsx(osv.osv_memory):
    _name = "ms.report.piutang.xlsx"
    _description = "Report Piutang XLSX"
    ...
    _columns = {
        ...
        'partner_ids': fields.many2many('res.partner', 'ms_report_piutang_customer_rel', 'ms_report_piutang_xlsx_id',
                                        'customer_id', 'Customer', copy=False, domain=[('customer','=',True)]),
    }
    ...
    def _print_excel_report(self, cr, uid, ids, data, context=None):
        ...
        filename = ('Report Piutang ') + str(date) + '.xlsx'
        ...
        query = """
            select rp.name, ai.number, aml.date, aml.date_maturity, aml.debit - aml.credit as total_piutang,
            case when aml.reconcile_partial_id is not null then aml3.debit - aml3.credit
            ...
            where ai.type = 'out_invoice' and a.type = 'receivable' and %s
            order by rp.name
        """ % (query_where)
        ...
        #WKS 1
        worksheet = workbook.add_worksheet('Piutang')
        ...
        worksheet.write('A2', 'Report Piutang' , wbf['title_doc'])
        ...
        worksheet.write('B%s' %(row), 'Customer' , wbf['header'])
        ...
        worksheet.write('F%s' %(row), 'Total Piutang' , wbf['header'])
        worksheet.write('G%s' %(row), 'Sisa Piutang' , wbf['header'])
        ...
        for res in result:
            cust = res[0] if res[0] else ''
            inv_number = res[1] if res[1] else ''
            date = res[2] if res[2] else ''
            due_date = res[3] if res[3] else ''
            tot_piutang = res[4] if res[4] else 0
            residual = res[5] if res[5] else 0
            not_due = res[5] if res[6] <= 0 else 0
            overdue_1_30 = res[5] if res[6]  >= 1 and res[6] <= 30 else 0
            overdue_31_60 = res[5] if res[6] >= 31 and res[6] <= 60 else 0
            overdue_61_90 = res[5] if res[6] >= 61 and res[6] <= 90 else 0
            overdue_lebih_90 = res[5] if res[6] > 90 else 0
            status = res[7] if res[7] else ''
            
            no += 1
            grand_tot_piutang += tot_piutang
            ...
            worksheet.write('B%s' %row, cust , wbf['content'])                    
            ...
            worksheet.write('F%s' %row, tot_piutang , wbf['content_float'])
        ...
        worksheet.write_formula(row-1,5, '{=subtotal(9,F%s:F%s)}' % (row1, row-1), wbf['total_float'], grand_tot_piutang)
        ...
        form_res = ir_model_data.get_object_reference(cr, uid, 'ms_report_piutang_xlsx_cst', 'view_report_piutang_xlsx')
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'ms.report.piutang.xlsx',
            'res_id': ids[0],
            'view_id': False,
            'views': [(form_id, 'form')],
