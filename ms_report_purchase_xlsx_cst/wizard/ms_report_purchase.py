...
class ms_report_purchase_xlsx(osv.osv_memory):
    _name = "ms.report.purchase.xlsx"
    ...
    _columns = {
        ...
        'partner_ids': fields.many2many('res.partner', 'ms_report_purchase_supplier_rel', 'ms_report_purchase_xlsx_id',
                                        'supplier_id', 'Supplier', copy=False, domain=[('supplier','=',True)]),
        'product_ids': fields.many2many('product.product', 'ms_report_purchase_product_rel', 'ms_report_purchase_xlsx_id',
                                        'product_id', 'Product', copy=False),
        'purchase_order_ids': fields.many2many('purchase.order', 'ms_report_purchase_purchase_order_rel', 'ms_report_purchase_xlsx_id',
                                        'purchase_id', 'Purchase Order', copy=False),
    }
    ...
    def _print_excel_report(self, cr, uid, ids, data, context=None):
        ...
        if purchase_order_ids :
            query_where +=" AND po.id in %s" % str(
                tuple(purchase_order_ids)).replace(',)', ')')
        if product_ids :
            query_where +=" AND pp.id in %s" % str(
                tuple(product_ids)).replace(',)', ')') 
        if state :
            query_where +=" AND po.state = '%s'" % str(state)
        
        query = """
            select po.name as po_number, po.date_order as po_date, rp.name as supplier, pp.name_template as prod_name, pav.name as grade,
            'lot' as lot, pol.product_qty as qty, pol.price_unit, pol.product_qty * pol.price_unit as subtotal, 'payment' as payment
            from purchase_order po
            left join purchase_order_line pol on pol.order_id = po.id
            left join product_product pp on pp.id = pol.product_id
            left join product_attribute_value_product_product_rel papp on papp.prod_id = pp.id
            left join product_attribute_value pav on pav.id = papp.att_id
        ...
        #WKS 1
        worksheet = workbook.add_worksheet('Purchase')
        ...
        filename = ('Purchase Report ') + str(date) + '.xlsx'
        
        #WKS 1
        worksheet.write('A1', company_name , wbf['company'])
        worksheet.write('A2', 'Purchase Report' , wbf['title_doc'])
        ...
        worksheet.write('G%s' %(row), 'Lot' , wbf['header'])
        worksheet.write('H%s' %(row), 'Quantity' , wbf['header'])
        worksheet.write('I%s' %(row), 'Price Unit' , wbf['header'])
        worksheet.write('J%s' %(row), 'Subtotal' , wbf['header'])
        worksheet.write('K%s' %(row), 'Payment' , wbf['header'])
        ...
        for res in result:
            po_name = res[0] if res[0] else ''
            po_date =res[1] if res[1] else ''
            supplier =res[2] if res[2] else ''
            prod_name =res[3] if res[3] else ''
            grade =res[4] if res[4] else ''
            lot = res[5] if res[5] else ''
            qty = res[6] if res[6] else ''
            price_unit = res[7] if res[7] else ''
            subtotal = res[8] if res[8] else ''
            payment = res[9] if res[9] else ''
            ...
            worksheet.write('B%s' %row, po_name , wbf['content'])                    
            worksheet.write('C%s' %row, po_date , wbf['content'])
            worksheet.write('D%s' %row, supplier , wbf['content'])  
            worksheet.write('E%s' %row, prod_name , wbf['content'])
            worksheet.write('F%s' %row, grade , wbf['content'])
            worksheet.write('G%s' %row, lot , wbf['content'])
            worksheet.write('H%s' %row, qty , wbf['content'])
            worksheet.write('I%s' %row, price_unit , wbf['content_float'])
            worksheet.write('J%s' %row, subtotal , wbf['content_float'])
            worksheet.write('K%s' %row, payment , wbf['content_float'])
        ...
        worksheet.write_formula(row-1,7, '{=subtotal(9,H%s:H%s)}' % (row1, row-1), wbf['total_float'], qty_total)
        worksheet.write('I%s' %row, '' , wbf['total_float'])
        worksheet.write_formula(row-1,9, '{=subtotal(9,J%s:J%s)}' % (row1, row-1), wbf['total_float'], total)
        ...
        form_res = ir_model_data.get_object_reference(cr, uid, 'ms_report_purchase_xlsx_cst', 'view_report_purchase_xlsx')
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'ms.report.purchase.xlsx',
            'res_id': ids[0],
            'view_id': False,
            'views': [(form_id, 'form')],
